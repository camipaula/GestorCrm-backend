...
const crearVenta = async (req, res) => {
  try {
    const { id_prospecto, objetivo, monto_proyectado, id_tipo_servicio } = req.body;
    ...
    const nuevaVenta = await VentaProspecto.create({
      ...
      id_tipo_servicio: id_tipo_servicio || null
    });
...
const editarObjetivoVenta = async (req, res) => {
  try {
    const { id_venta } = req.params;
    const { objetivo, monto_proyectado, id_tipo_servicio } = req.body;
    ...
    if (id_tipo_servicio !== undefined) {
      venta.id_tipo_servicio = id_tipo_servicio;
    }
